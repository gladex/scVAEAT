...
from data_utils import AnnDataSet, balancer
...
class SCVAEAT(nn.Module):
    def __init__(
        ...
        self.attnencoder = ATTN(dff_size, model_size).to(self.device)
    ...
    def get_latent_adata(self, adata):
        x = Tensor(adata.to_df().values).to(self.device)
        latent_z = self.encode(x)[0].detach().cpu().numpy()
        return sc.AnnData(X=latent_z, obs=adata.obs.copy())
    ...
    def train_scVAEAT(self, train_adata, epochs=100, batch_size=128, lr=5e-4):
        ...
        for epoch in pbar:
            ...
            for x in train_loader:
                ...
                loss_smooth = torch.nn.functional.smooth_l1_loss(
                    x, x_hat, reduction="none"
                ).sum(dim=1)
                total_loss = (
                    0.5 * loss_rec
                    + 0.5 * (loss_kl * self.kl_weight)
                    + 0.01 * loss_smooth
                ).mean()
            ...
            pbar.set_postfix(
                SCPRAM_loss=float(total_loss.item()),
                recon_loss=float(loss_rec.mean().item()),
                kl_loss=float(loss_kl.mean().item()),
                smooth_l1_loss=float(loss_smooth.mean().item()),
            )
    ...
    def ot_predict(self, adata_train, cell_to_pred, key_dic):
        ...
        pred_x = self.decode(Tensor(z_pred).to(self.device)).detach().cpu().numpy()
    ...
    def cross_cell_predict(self, train_adata, cell_to_pred, key_dic, n_top=None):
        ctrl_adata = train_adata[
            (train_adata.obs[key_dic["cell_type_key"]] == cell_to_pred)
        ...
        cos_sim = cosine_similarity(
            np.array(test_z).reshape(-1, self.latent_dim),
        ...
        if n_top is None:
            ...
            delta_pred = np.sum(
                ...
                * np.array(delta_list).reshape(-1, self.latent_dim)[top_indices],
                axis=0,
            )

        pred_z = test_z + delta_pred
        pred_x = self.decode(Tensor(pred_z).to(self.device)).detach().cpu().numpy()
        pred_adata = sc.AnnData(X=pred_x, obs=ctrl_adata.obs.copy(), var=ctrl_adata.var.copy())
        pred_adata.obs[key_dic["condition_key"]] = key_dic["pred_key"]
        return pred_adata

    def predict(self, train_adata, cell_to_pred, key_dic, ratio=0.05):
        ctrl_to_pred = train_adata[
            (train_adata.obs[key_dic["cell_type_key"]] == cell_to_pred)
            & (train_adata.obs[key_dic["condition_key"]] == key_dic["ctrl_key"])
        ]
        ctrl_adata = train_adata[
        ...
        G = ot.emd(
            torch.ones(stim.shape[0]) / stim.shape[0],
